...
export class AudioRecord {
	...
	private mimeType: SupportedMimeType = pickMimeType(
	...
	async handlePauseResume() {
		if (!this.mediaRecorder || this.mediaRecorder.state === "inactive") {
			...
			throw new Error(
				"There is no mediaRecorder, cannot handlePauseResume",
			);
		}
	...
	async resumeRecording() {
		if (!this.mediaRecorder || this.mediaRecorder.state === "inactive") {
			console.error(
				"There is no mediaRecorder, cannot resume resumeRecording",
			);
			throw new Error(
				"There is no mediaRecorder, cannot resumeRecording",
			);
		}
